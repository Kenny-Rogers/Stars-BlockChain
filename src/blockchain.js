        ...
        return new Promise(async (resolve, reject) => {
            try{
                ...
                if(self.chain.length>0){
                    block.previousBlockHash = this.chain[self.chain.length-1].hash;
                }
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise(async (resolve, reject) => {
            for (let index = 1; index < self.chain.length; index++) {
                const star = await self.chain[index].getBData();
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            for (let index = 1; index < self.chain.length; index++) {
                ...
                if(isValid){
                    if(previousBlockHash != self.chain[index].previousBlockHash){
                        errorLog.push(`Block ${block.hash} has invalid previous block hash`);
                    }
